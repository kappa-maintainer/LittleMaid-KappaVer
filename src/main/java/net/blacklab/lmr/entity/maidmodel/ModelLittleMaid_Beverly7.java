...
public class ModelLittleMaid_Beverly7 extends ModelLittleMaidBase {
	...
	public float getConditionalHeight(boolean motionSitting, boolean isSneak, boolean isWait, boolean ... others) {
		if(others == null)return 0.0F;
		if(others.length == 0)return 0.0F;
		if(isSneak && !isWait && !motionSitting) {
			return 1.8125F;
		}
		if(motionSitting || isSneak && isWait || !others[0] && isWait) {
			if(!isSneak) {
				return 1.75F;
			}
			return 1.625F;
	...
	public void setRotationAngles(float f, float f1, float ticksExisted, float pheadYaw, float pheadPitch, float f5, IModelCaps pEntityCaps)
	{
		...
		else
		{
			if (isSneak || !airBlockOnTop || motionSitting) //���Ⴊ�� ���㒍��
			{
				...
				{//����
					...
					hipBody.rotateAngleX -= 0.1F + (float)Math.sin(ticksExisted * 0.057F) * 0.03F;
					upperRightArm.rotateAngleX += 0.1F;
					upperLeftArm.rotateAngleX += 0.1F;
				...
				//���Ⴊ�ݕ��s
				float f15 = (float)Math.sin(f * 0.6565F); //wave1
				float f16 = (float)Math.cos(f * 0.6565F); //wave2
				...
				breastR.rotateAngleX -= f16 * f16 * 0.18F * f1 - (float)Math.sin(ticksExisted * 0.057F) * 0.05F;
				breastL.rotateAngleX -= f16 * f16 * 0.18F * f1 - (float)Math.sin(ticksExisted * 0.057F) * 0.05F;
				mainFrame.rotationPointY += f16 * f16 * 0.5F;
			}
			else
			{
				//�ʏ���s
				float f15 = (float)Math.sin(f * 0.4444F); //wave1
				float f16 = (float)Math.cos(f * 0.4444F); //wave2
				float f22 = f15 > f16 ? f15 : f16; //upper wave
				float f35 = f15 < f16 ? f15 : f16; //lower wave
		...
		{// �ҋ@��� �r
			...
			Arms[0].rotateAngleY -= 0.5F;
			Arms[0].rotateAngleZ += 0.1F;
			upperLeftArm.rotateAngleX += (float)Math.sin(ticksExisted * 0.057F) * 0.05F - 0.5F;
			upperLeftArm.rotateAngleZ += 0.3F;
			Arms[1].rotateAngleX += 1.7F;
			Arms[1].rotateAngleY += 0.5F;
			Arms[1].rotateAngleZ -= 0.1F;
		...
		else
		{
			...
			{// �|�\�� �r
				...
				upperRightArm.rotateAngleZ += (float)Math.cos(ticksExisted * 0.08F) * 0.03F + 0.05F;
				upperLeftArm.rotateAngleZ -= (float)Math.cos(ticksExisted * 0.08F) * 0.03F + 0.05F;
				upperRightArm.rotateAngleX += (float)Math.sin(ticksExisted * 0.057F) * 0.05F;
				upperLeftArm.rotateAngleX -= (float)Math.sin(ticksExisted * 0.057F) * 0.05F;
		...
		//�r���Ǐ]
		float sinBody1X = (float)Math.sin(bipedBody.rotateAngleX);
		float cosBody1X = 1F - (float)Math.cos(bipedBody.rotateAngleX);
		float sinBody2X = (float)Math.sin(bipedBody.rotateAngleX + hipBody.rotateAngleX);
		float cosBody2X = 1F - (float)Math.cos(bipedBody.rotateAngleX + hipBody.rotateAngleX);
		float sinBody2Y = (float)Math.sin(bipedBody.rotateAngleY + hipBody.rotateAngleY);
		float cosBody2Y = 1F - (float)Math.cos(bipedBody.rotateAngleY + hipBody.rotateAngleY);
