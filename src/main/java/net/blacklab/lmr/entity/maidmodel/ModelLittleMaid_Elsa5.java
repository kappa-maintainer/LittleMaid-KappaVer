...
public class ModelLittleMaid_Elsa5 extends ModelLittleMaidBase {
	...
	public void setRotationAngles(float f, float f1, float ticksExisted, float pheadYaw, float pheadPitch, float f5, IModelCaps pEntityCaps)
	{
		...
		bipedLeftLeg.rotateAngleX -= (float)Math.cos(f * 0.5656F) * 1.2F * f1;
		Skirt.rotateAngleY += (float)Math.cos(f * 0.5656F) * 0.15F * f1;
		hemSkirt.rotateAngleY += (float)Math.cos(f * 0.5656F) * 0.25F * f1;
		...
		if (isSneak)
		{
			...
			bipedHead.rotationPointZ -= upperBodyLength * (float)Math.sin(bipedBody.rotateAngleX);
			bipedHead.rotationPointY += upperBodyLength * (1 - (float)Math.cos(bipedBody.rotateAngleX));
			bipedRightLeg.rotationPointZ += lowerBodyLength * (float)Math.sin(bipedBody.rotateAngleX);
			bipedLeftLeg.rotationPointZ += lowerBodyLength * (float)Math.sin(bipedBody.rotateAngleX);
			bipedRightLeg.rotationPointY -= lowerBodyLength * (1 - (float)Math.cos(bipedBody.rotateAngleX));
			bipedLeftLeg.rotationPointY -= lowerBodyLength * (1 - (float)Math.cos(bipedBody.rotateAngleX));
			Skirt.rotationPointZ += lowerBodyLength * (float)Math.sin(bipedBody.rotateAngleX);
			Skirt.rotationPointY -= lowerBodyLength * (1 - (float)Math.cos(bipedBody.rotateAngleX));
			mainFrame.rotationPointY += lowerBodyLength * (1 - (float)Math.cos(bipedBody.rotateAngleX));
		}
		else
		{
			// �ʏ헧��
		...
		if (isWait)
		{
			...
			bipedLeftArm.rotateAngleX += (float)Math.sin(ticksExisted * 0.062F) * 0.05F -0.6F;
		...
		else
		{
			if (aimedBow)
			{
				...
				bipedLeftArm.rotateAngleZ -= (float)Math.cos(ticksExisted * 0.09F) * 0.05F + 0.05F;
				bipedRightArm.rotateAngleX += (float)Math.sin(ticksExisted * 0.062F) * 0.05F;
				bipedLeftArm.rotateAngleX -= (float)Math.sin(ticksExisted * 0.062F) * 0.05F;
